...
class WidgetModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
    ...
    fun forceUpdateAllWidgets(promise: Promise?) {
        try {
            ...
            Log.d(TAG, "Force update broadcast sent for all widgets")
            promise?.resolve(true)
        } catch (e: Exception) {
            Log.e(TAG, "Error forcing widget update", e)
            promise?.reject("WIDGET_ERROR", "Failed to force widget update: ${e.message}")
        }
    }
    ...
    @ReactMethod
    fun setWidgetDirectValues(userId: String, narrative: String, colorHex: String, promise: Promise? = null) {
        try {
            val context = reactApplicationContext
            val prefs = context.getSharedPreferences("MoodWidgetPrefs", Context.MODE_PRIVATE)
            val editor = prefs.edit()
            
            // Save direct values for faster widget access
            editor.putString("user_mood_narrative", narrative)
            editor.putString("user_mood_color", colorHex)
            editor.putLong("user_mood_updated", System.currentTimeMillis())
            ...
            Log.d(TAG, "Direct widget values set. Narrative: $narrative, Color: $colorHex")
            
            // Force update widgets to show the new values
            forceUpdateAllWidgets(promise)
            
            promise?.resolve(true)
        } catch (e: Exception) {
            Log.e(TAG, "Error setting direct widget values: ${e.message}", e)
            promise?.reject("ERROR", "Failed to set direct widget values: ${e.message}")
        }
    }
    
    @ReactMethod
    fun setFriendDirectValues(friendId: String, friendName: String, narrative: String, colorHex: String, promise: Promise? = null) {
        try {
            val context = reactApplicationContext
            val prefs = context.getSharedPreferences("MoodWidgetPrefs", Context.MODE_PRIVATE)
            val editor = prefs.edit()
            ...
            // Save direct values for faster widget access
            editor.putString("friend_${friendId}_mood_narrative", narrative)
            editor.putString("friend_${friendId}_mood_color", colorHex)
            editor.putString("friend_${friendId}_name", friendName)
            editor.putLong("friend_${friendId}_mood_updated", System.currentTimeMillis())
            
            editor.apply()
            
            Log.d(TAG, "Direct widget values set for friend $friendName. Narrative: $narrative, Color: $colorHex")
            ...
            Log.e(TAG, "Error setting direct widget values for friend: ${e.message}", e)
            promise?.reject("ERROR", "Failed to set direct widget values for friend: ${e.message}")
        }
    }

    @ReactMethod
    fun logWidgetData(userId: String, promise: Promise? = null) {
        try {
            val context = reactApplicationContext
            val prefs = context.getSharedPreferences("MoodWidgetPrefs", Context.MODE_PRIVATE)
            
            // Log the key data needed by widgets
            val narrative = prefs.getString("user_mood_narrative", null) ?: "not set"
            val color = prefs.getString("user_mood_color", null) ?: "not set"
            val rawData = prefs.getString("user_color_mix_data", null) ?: "not set"
            
            Log.d(TAG, "Widget data for $userId:")
            Log.d(TAG, "- narrative: $narrative")
            Log.d(TAG, "- color: $color")
            Log.d(TAG, "- Last updated: ${prefs.getLong("user_color_mix_updated", 0)}")
            Log.d(TAG, "- Raw data available: ${rawData != "not set"}")
            
            promise?.resolve(true)
        } catch (e: Exception) {
            Log.e(TAG, "Error logging widget data: ${e.message}", e)
            promise?.reject("ERROR", "Failed to log widget data: ${e.message}")
        }
    }

    @ReactMethod
    fun refreshFriendWidgets(friendId: String, promise: Promise?) {
        try {
            ...
            for (providerClass in widgetProviders) {
                val componentName = android.content.ComponentName(reactApplicationContext, providerClass)
                intent.component = componentName
                reactApplicationContext.sendBroadcast(intent)
            }
            
            Log.d(TAG, "Friend widget refresh broadcast sent for friendId: $friendId")
            promise?.resolve(true)
        } catch (e: Exception) {
            Log.e(TAG, "Error refreshing friend widgets", e)
            promise?.reject("WIDGET_ERROR", "Failed to refresh friend widgets: ${e.message}")
        }
    }
} 